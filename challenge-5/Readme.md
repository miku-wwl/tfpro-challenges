...
本实验使用 LocalStack，因此请在 Terraform 之外使用 AWS CLI 创建用于保存
Backend State 的 Bucket：
...
该 Bucket 必须先于 Terraform S3 Backend 创建，并且不应在 Terraform 配置中定义
为 `aws_s3_bucket` 资源，否则会产生 Backend 初始化依赖问题。真实 AWS 环境中使用
相同的 `aws s3api` 命令时，可以去掉 `--endpoint-url`。

在 `infra/vpc-infra` 目录中：

* 使用 module source 调用 `vpc` 子模块。
* 配置 S3 backend，将 state 文件存储为 `vpc.tfstate`。
* 运行 `terraform init -reconfigure` 初始化 S3 backend。
* 导入现有 VPC 和子网资源，使其写入 S3 backend 中的 `vpc.tfstate`。
* 将本地 state 迁移到 S3 backend。

> [!NOTE]
> 使用 `terraform import` 导入带有 `for_each` 字符串索引的资源时，Linux/macOS
> 通常使用单引号保护完整地址，例如：
> `terraform import 'module.vpc.aws_subnet.challenge_5["subnet1"]' <SUBNET_ID>`。
> PowerShell 对嵌套引号的处理容易导致 Terraform 收到错误的资源地址；建议改用
> `import` block，或按 PowerShell 的转义规则传递地址，并在导入后运行
> `terraform state list` 和 `terraform plan` 验证结果。

### 任务 10：导入 EC2 和 SG 基础设施

在 `infra/others` 目录中：

开始本任务前，先在 `vpc` 子模块中定义 `subnet_ids` output，并在
`infra/vpc-infra` 中运行 `terraform apply` 或刷新 state，使该 output
写入 S3 backend 的 `vpc.tfstate`。这是后续 `terraform_remote_state`
读取子网 ID 的前置条件。
