...
这里的 `local_file.this` 是根模块中的完整资源地址。PowerShell 中建议将整个
`-target` 参数加引号，避免 Terraform 将资源类型和名称错误拆开。
...
```powershell
terraform state show module.compute.aws_launch_template.this
terraform state show module.compute.terraform_data.desired_capacity
terraform state show module.iam.aws_iam_user.lb
terraform state show module.iam.aws_iam_user_policy.lb_ro
```
